...
import numpy as np
import time
from MODES.text import generate_Text_Lock, detect_info_1
from MODES.capture import generate_Capture_Lock, detect_info_2
from MODES.video import generate_Video_Lock, detect_info_3
from MODES.live import generate_Live_Lock, detect_info_4
import json
import sys_info
...
@app.route('/info')
def info():
	global detect_info_1, detect_info_2, detect_info_3, detect_info_4
	lis_t  = ['allow_Text_Mode', 'allow_Capture_Mode', 'allow_Video_Mode', 'allow_Live_Mode']
	for x_ in range(len(lis_t)):
		if globals()[lis_t[x_]]:
			fire_n_smoke_val = globals()['detect_info_' + str(x_ + 1)]
	info_ = sys_info.log_gps_and_network_data()
	return jsonify({
		'fire_n_smoke':	fire_n_smoke_val,
		'info': info_
	})
...
def video_feed():
	if not allow_Live_Mode:
		return Response(json.dumps('CaptureMode_disabled'), mimetype='text/event-stream')
	return Response(generate_Live_Lock(globals()),
